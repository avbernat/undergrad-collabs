...
def get_IDs(filepath):
    ...
    with open(filepath, "r") as data_file:
        ...
        for row in reader:
            ...
            if row["filename"] != 'NA':  #no filename column
                filename_count += 1
...
cls()
# Autumn 2019
#path = "/Users/anasilberg/Desktop/Standardized Peaks Test/"
...
# Winter 2020
folderpath = f"/Users/{username}/Desktop/git_repositories/undergrad-collabs/voltage_noise"
...
##            OutputFile=open(r"/Users/anasilberg/Desktop/Flight_Analyses/" + filename, "w")
##            for index in range(0, len(time_graph)):
##                OutputFile.write('%.1f' % time_graph[index] + ',' + '%.2f' %speed_graph[index] + '\n')
##            OutputFile.close()
        
##        with open(r"/Users/anasilberg/Desktop/flight_stats4/flight_stats-" + str(file).split("_")[-1].replace(".txt", "") + ".csv", "w") as csv_file:
